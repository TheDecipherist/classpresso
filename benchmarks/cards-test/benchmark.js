/**
 * Classpresso Cards Benchmark
 * Tests a realistic dashboard with 100 varied cards (10 different types)
 */
...
function printResults(baseline, optimized) {
  console.log('\n' + '='.repeat(80));
  console.log('CLASSPRESSO CARDS BENCHMARK - 100 VARIED CARDS');
  console.log('='.repeat(80));
  console.log(`Card types: 10 different layouts | Runs per test: ${RUNS_PER_TEST} | CPU Throttle: ${CPU_THROTTLE}x`);
  console.log('='.repeat(80));
  ...
  const fcpDiff = baseline.firstContentfulPaint - optimized.firstContentfulPaint;
  const fcpPct = (fcpDiff / baseline.firstContentfulPaint * 100).toFixed(1);
  console.log(`First Contentful Paint: ${fcpDiff > 0 ? 'improved' : 'changed'} by ${fcpPct}%`);

  console.log('\n');
}

async function main() {
  console.log('Classpresso Cards Benchmark');
  console.log('100 varied cards (10 different types)');
  console.log('='.repeat(50));

  // Generate fresh page
  console.log('\n[1/5] Generating cards page...');
  execSync('node generate-cards.js', { cwd: __dirname, stdio: 'inherit' });
  ...
  console.log(`  HTML: ${baseline.htmlSize.toFixed(2)} KB, FCP: ${baseline.firstContentfulPaint.toFixed(0)}ms`);
  ...
  console.log(`  HTML: ${optimized.htmlSize.toFixed(2)} KB, FCP: ${optimized.firstContentfulPaint.toFixed(0)}ms`);
  ...
  fs.writeFileSync(resultsFile, JSON.stringify({
    ...
    config: { cards: 100, cardTypes: 10, runsPerTest: RUNS_PER_TEST, cpuThrottle: CPU_THROTTLE },
    baseline,
    optimized,
  }, null, 2));
